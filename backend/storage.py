...
def get_times(difficulty: str, count=5) -> list:
    ...
    with connect() as cur:
        query = "SELECT time, swedish, difficulty FROM times WHERE difficulty = ? ORDER BY RANDOM() LIMIT ?"
        cur.execute(query, (difficulty, count))
        results = cur.fetchall()

        words = []
        for result in results:
            query = "SELECT swedish FROM times WHERE difficulty = ? ORDER BY RANDOM() LIMIT 3"
